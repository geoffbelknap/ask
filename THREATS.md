...
| **Parameter-to-prompt injection (P2P)** | Novel | AML.T0051 | The agent's own invocation surface is the injection vector: attacker-controlled instructions are embedded in a URL query parameter, deep link, or webhook payload that launches the agent (e.g. a `?q=` search parameter). The victim clicks a link and the agent executes the embedded prompt with the victim's authority. The attack arrives through the front door, not through retrieved content. | High — the malicious instruction is delivered as a normal-looking link and runs with the victim's full ambient authority over connected data sources. Demonstrated in M365 Copilot SearchLeak (CVE-2026-42824). Relates to Tenet 24 — the invocation channel is data, not a principal instruction. |
