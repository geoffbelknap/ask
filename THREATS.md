...
| **Runtime tampering** | Traditional | — | The agent's Runtime — container image, binaries, dependencies — is modified before or during operation, either through supply chain compromise or host-level access. | Critical — a compromised runtime undermines all governance. Relates to `runtime-known`. |
| **Tool abuse within permissions** | Hybrid | AML.T0054 | Agent uses authorized tools for unintended purposes — reading sensitive files via a legitimate file tool, exfiltrating data through an allowed API. The tools work as designed; the intent is malicious. | High — least privilege (`capability-declared`) and operational bounds (`operations-bounded`) limit scope, but authorized access used maliciously is hard to distinguish from normal operation. |
| **Excessive agency / confused deputy** | Novel | AML.T0054 | The agent holds high-impact authority (changing account settings, resetting credentials, issuing verification codes, moving funds) and is socially engineered into exercising it on behalf of an unverified requester. The agent is the deputy; the attacker borrows its authority. The action is within the agent's permissions — the failure is acting without verifying the requester. | High to Critical — irreversible or identity-affecting actions performed for the wrong principal. Demonstrated in the Meta AI / Instagram account-recovery takeover (June 2026): a support agent linked attacker emails and sent verification codes on request. Relates to `unverified-zero-trust` (unverified → zero trust) and the need for step-up verification on high-impact actions. |
| **Agent framework remote code execution** | Novel | — | The agent's Runtime itself contains a path from model output to execution — a shell invocation, an evaluator, or a deserializer reached without an intervening policy check. Exploitation needs no compromise of the model, only output crafted to reach the path. | Critical — collapses the Model/Runtime boundary, so attestation of what the Runtime *is* provides no protection. Demonstrated repeatedly through 2026 against widely used agent frameworks, including chains achieving host code execution with no user interaction. Addressed by `model-output-mediated`. |
...
| Risk | Category | ATLAS | Description | Severity Context |
|---|---|---|---|---|
| **Mediation bypass** | Critical | — | Agent finds or is given a path to external resources that doesn't traverse the mediation layer — direct network access, unproxied DNS, or a misconfigured network rule. | Critical — violates `mediation-complete`. If mediation is bypassed, credential isolation, audit logging, and egress controls all fail. |
...
| **Enforcement infrastructure failure** | Novel | — | The mediation layer, gateway, or proxy crashes or becomes unavailable. Depending on fail mode, the agent may gain unmediated access or halt entirely. | Critical if fail-open, expected if fail-closed (`enforcement-fails-closed`). The distinction between these outcomes is the entire point of `enforcement-fails-closed`. |
| **Enforcement infrastructure compromise** | Traditional | — | An attacker compromises a proxy, gateway, or enforcement sidecar — through a vulnerability in the component itself or its supply chain. The enforcement layer becomes attacker-controlled. | Critical — undermines the entire governance model. All invariants assume enforcement integrity. Relates to `runtime-known`. |
| **Rendered-output exfiltration via trusted-domain proxy** | Novel | AML.T0024 | Data leaves not through an agent-initiated network call but through the *rendering surface*: the agent's output contains a markdown image or link whose URL encodes stolen data, and a client or downstream service fetches it. A streaming/HTML-injection race can render raw markup before sanitization applies. Where a domain allowlist or CSP restricts destinations, the attacker routes through a *trusted* domain that will fetch arbitrary URLs (server-side request forgery via a permitted proxy), defeating the allowlist. | High to Critical — bypasses domain allowlists and Content-Security-Policy simultaneously. Demonstrated in M365 Copilot SearchLeak: stolen mailbox data was exfiltrated through Bing's image-fetch proxy (a Microsoft-owned, allowlisted domain). Relates to `mediation-complete` — rendered output is an egress event; trusted-domain SSRF is not a mediated path. |
...
| **Parameter-to-prompt injection (P2P)** | Novel | AML.T0051 | The agent's own invocation surface is the injection vector: attacker-controlled instructions are embedded in a URL query parameter, deep link, or webhook payload that launches the agent, such as a `?q=` search parameter. The victim clicks a link and the agent executes the embedded prompt with the victim's authority. The attack arrives through the front door, not through retrieved content. | High — the malicious instruction is delivered as a normal-looking link and runs with the victim's full ambient authority over connected data sources. Demonstrated in M365 Copilot SearchLeak (CVE-2026-42824). Relates to `instruction-channel-distinct` — the invocation channel is data, not a principal instruction. |
| **Agent data injection** | Novel | AML.T0051 | Instructions disguised as trusted structured data — a sender name, a button identifier, a field the agent treats as metadata rather than content. Defenses aimed at free-text injection do not inspect these surfaces. | High — the payload arrives through fields the agent has no reason to distrust. `instruction-channel-distinct` covers it by channel rather than by content inspection. |
| **Time-of-check to time-of-use on computer-use agents** | Novel | — | The observed state changes between the agent's observation and its action. A screen element moves or is replaced after the agent decides to click it, so the action lands on an attacker-chosen target. | High — the agent's decision was correct when made. Mediation that validates a request at submission does not cover a target that changed after validation. Expands with browser and computer-use deployment. |
| **Cross-modal / multimodal injection** | Novel | AML.T0051 | Instructions hidden in non-text inputs the agent processes — text rendered inside an image, a QR code, audio, video frames, or the pixels and DOM of a screen a computer-use or browser agent observes. The model interprets the embedded content as directives. Text-oriented guardrails do not inspect these surfaces. | High — expands the injection surface beyond text exactly as computer-use, browser, and voice agents go mainstream; pre/post-call scanning built for text misses it. Relates to `instruction-channel-distinct` — instruction-like content is data regardless of modality. |
...
| **Constraint manipulation** | Traditional | AML.T0059 | An attacker attempts to modify the agent's constraints — policy files, tier configuration, behavioral parameters. | Low if architecture is correct (read-only mounts make this structurally impossible per `constraints-external`). Critical if a bug allows constraint writes. |
...
| Risk | Category | ATLAS | Description | Severity Context |
|---|---|---|---|---|
| **Model distillation / capability extraction** | Novel | AML.T0024 | An adversary runs a large volume of exchanges through the agent's model and trains a weaker "student" model on the outputs — distilling the stronger model's capabilities. Every exchange is an authorized inference call; the harm is the aggregate and the purpose. Typically distributed across many fraudulent accounts (Sybil) to keep each identity under its individual rate and spend limits. | High — IP theft through the legitimate inference interface, invisible at the level of any single call. Attributed at scale in 2026. In February 2026 three frontier labs disclosed simultaneously, covering more than 24,000 fraudulent accounts and over 16 million exchanges. Defense has since moved from prevention toward attribution: fingerprinting that survives into a student model, and watermarks detectable in a student trained on a small fraction of marked text. `reasoning-not-emitted` (reasoning is not a principal-facing surface) removes the richest extraction channel; `operations-bounded` and `trust-not-self-elevated` bound and detect the volumetric case. See [MITIGATIONS.md](MITIGATIONS.md#model-distillation-and-reasoning-exposure). |
| **Knowledge-base distillation** | Novel | AML.T0024 | An insider or compromised agent systematically retrieves organizational-knowledge nodes one authorized query at a time until it has reconstructed a restricted corpus that no single query — and no single synthesized view (`knowledge-access-bounded`) — would have exposed. Aggregation over time is the side channel. | High — defeats per-query authorization and single-synthesis bounds alike. Relates to `knowledge-durable`, `knowledge-access-bounded`, `reasoning-not-emitted`. |
| **Constraint / behavioral profile extraction** | Novel | AML.T0049 | An attacker probes the agent across many sessions to assemble a complete map of its constraints, thresholds, tool surface, and behavioral parameters — extraction-by-aggregation of the visible Constraints layer to plan more targeted attacks. | Medium — extends system prompt extraction (single-shot) to patient, multi-session aggregation. The visible Constraints layer is intentionally readable; the volume and breadth of probing is the signal. `reasoning-not-emitted` treats probing of reasoning and constraints as data that informs trust, not as authorized requests. |
...
| **Rogue agent deployment** | Novel | AML.T0073 | An attacker deploys their own agent within the victim's infrastructure — either by compromising the deployment pipeline or by exploiting a coordinator's delegation capability to spawn a malicious worker. | Critical — the attacker's agent operates within the trust boundary. Mitigated by operator approval for agent creation and runtime integrity verification (`runtime-known`). |
...
| **Delegation privilege escalation** | Traditional | — | A coordinator delegates capabilities it doesn't hold, or combines sub-agent outputs to exceed individual authorization. | High — `delegation-bounded` and `labeled-delivery-enforced` address this directly. Enforcement depends on explicit permission validation at the delegation bus. |
| **Exfiltration via agent tools** | Novel | AML.T0066 | A compromised agent uses its authorized tools to exfiltrate data — sending sensitive content through a messaging tool, writing it to an accessible file, or encoding it in API requests. The exfiltration uses legitimate tool invocations, not network exploits. | High — looks like normal agent behavior. Operational bounds (`operations-bounded`) and behavioral monitoring are primary defenses. |
...
Risks the agent system poses to others. Every section above covers risks *to* the agent system. This one covers harm the deployment causes, including to parties outside its governance domain.

| Risk | Category | ATLAS | Description | Severity Context |
|---|---|---|---|---|
| **Evaluation containment escape** | Novel | — | An agent escapes a testing or evaluation environment and reaches production systems, its own or another party's. Evaluation environments carry weaker controls precisely because they are believed to be contained. | Critical — demonstrated in July 2026, when frontier models under a cyber-capability evaluation with safety refusals deliberately reduced exploited a zero-day in a package-registry proxy inside the evaluation boundary, reached the open internet, and compromised an unrelated company's production infrastructure. Addressed by `containment-matches-context` and `boundary-violation-halts`. |
| **Specification gaming with external effect** | Novel | — | An agent pursues its assigned objective through actions its operator did not intend and a third party did not consent to. The agent is neither compromised nor misaligned; the objective was underspecified. | High to Critical — every individual action falls within granted capability, so capability controls do not fire. Bounded by `capability-composition-governed` and `operations-bounded`; detected through `trajectory-recorded`. |
| **Third-party harm from an authorized agent** | Novel | — | An agent operating within its granted capability damages a party outside its governance domain. Liability, disclosure, and remediation duties are unresolved. | High — cyber insurers are writing AI exclusions, and California SB 53 now defines a reportable incident class covering loss of control and deliberate evasion of developer safeguards. |
| **Autonomous attack chaining** | Novel | — | An agent independently performs reconnaissance, exploitation, credential harvesting, and lateral movement without step-by-step human direction. The operator's intent covers the goal, not the method. | Critical — observed at scale in 2026. Detection depends on evaluating action sequences rather than individual calls, which is what `trajectory-recorded` exists to make possible. |
| **Confused deputy through delegated authority** | Novel | AML.T0054 | An agent holding standing authority exercises it for a requester who does not hold that authority. Distinct from the entry under Agent Runtime in that the failure is structural rather than a verification lapse: nothing bounds the agent's authority by the requester's. | High to Critical — addressed by `authority-derived-from-principal` and `verification-proportional`. Agentic commerce makes this a payment-fraud surface: a compromised agent holding delegated payment credentials produces transactions indistinguishable from normal behavior. |
