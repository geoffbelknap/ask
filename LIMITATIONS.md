...
**The visible Superego is an attack surface map.** A compromised agent can read `mind.yaml` to learn its policy boundaries: which tools are denied, escalation thresholds, delegation limits. The real enforcement is in the invisible Superego (proxy configs, guardrail rules, gateway policy) — knowing the threshold doesn't help bypass the proxy that enforces it. Operators should put only what the agent needs for self-governance in the visible Superego.
