...
5. **Constraints are operator-owned and read-only.** Anything that governs what the agent is permitted to do (policy, guardrail rules, tier config, AGENTS.md) belongs in the Constraints layer and must be read-only to the agent.
6. **Each enforcement layer has its own isolation boundary.** Network isolation, egress proxy, LLM proxy, enforcer, container hardening, and runtime gateway are separate containers/mechanisms. Don't collapse enforcement layers into a single container or let one component subsume another's role.
