...
ALL communication between agent and external systems MUST pass through proxies the agent cannot bypass or disable. No path from agent to any external resource may bypass mediation. The condition is binary. Mediation is complete, or the framework has failed.
