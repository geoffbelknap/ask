...
Tests are keyed by property name. Each test also carries its current `INV` number, so a number is searchable here and in [FRAMEWORK.md](FRAMEWORK.md) alike. Numbers reflect reading order and can change; names do not. The register build (`scripts/build-invariants-page.py`) fails if the numbers here disagree with FRAMEWORK.md.
...
**INV-11 — Output provenance is applied by the [mediation layer](GLOSSARY.md#elements-and-layers).** `provenance-mediated`
