...
- **Invariant contradictions or ambiguity.** Cases where two invariants conflict, or where an invariant could be interpreted in incompatible ways.
- **Real-world testing feedback.** If you build against ASK, report what works, what doesn't, and what's missing.
- **Regulatory mapping.** Contributions mapping ASK invariants to specific regulatory requirements (EU AI Act, NIST AI RMF, SOC 2, HIPAA, GDPR).
...
- If modifying invariants: explicit rationale for why the change strengthens the framework
...
Invariants are the most stable part of the framework. Changing an invariant has cascading implications across the entire project. To propose an invariant change:

1. **Open an issue first** — describe the problem the current invariant creates, not just the change you want
2. **Show the impact** — which other invariants, documents, or architectural patterns are affected
3. **Demonstrate the gap** — provide a concrete scenario where the current invariant fails
4. **Propose the property** — state the new property that must hold, not the implementation

Invariant additions, modifications, and removals require maintainer approval. Reference invariants by slug. The `INV-nn` number reflects reading order, carries no meaning, and changes when the framework is reorganized.

## Governance

ASK is currently maintained by [Geoff Belknap](https://github.com/geoffbelknap). As the project matures, governance may expand to include additional maintainers. Decisions on invariant changes, structural reorganization, and major direction are made by the maintainer(s) with input from the community.
